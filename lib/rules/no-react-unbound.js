...
module.exports = {
  meta: {
    docs: {
      description:
        "Prevent using unbound class method as callback of a JSX function",
      category: "Fill me in",
      recommended: false
    },
  ...
  create(context) {
    ...
    function match(ast, condition) {
      ...
      Traverser.traverse(ast, {
        enter(node, parent) {
          let nodeWithParent = Object.assign({}, node, { parent });
    ...
    return {
      MethodDefinition(classMethodNode) {
        ...
        const unboundCallsOfTheMethod = match(
          classMethodNode.parent,
          descendantNode => {
            ...
            if (descendantNode.property.name !== classMethodNode.key.name)
              return false;
